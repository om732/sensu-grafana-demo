  ...
  def run
    ...
    `df -PB1 #{config[:local] ? '-l' : ''}`.split("\n").drop(1).each do |line|
      ...
      unless %r{/sys|/dev|/run}.match(mnt)
        ...
        mnt = mnt.gsub '/', delim
        output [config[:scheme], mnt, 'total'].join('.'), total.gsub('K', '')
        output [config[:scheme], mnt, 'used'].join('.'), used.gsub('K', '')
        output [config[:scheme], mnt, 'avail'].join('.'), avail.gsub('K', '')
